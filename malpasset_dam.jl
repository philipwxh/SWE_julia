...
function swe_2d_rhs(U,ops,dis_cst,vgeo,fgeo,nodemaps, dt, tol, g, pre_allo)
    ...
    @batch for i in mapB
        vu_dot_n  = huP[i]*nxJ[i]+ hvP[i]*nyJ[i];
        vu_dot_n = vu_dot_n/(nxJ[i].^2 + nyJ[i].^2);
        huP[i] =  huP[i] - 2*vu_dot_n*nxJ[i];
        hvP[i] =  hvP[i] - 2*vu_dot_n*nxJ[i];
    end
    @batch for e = 1:nelem
        for i = 1:Nfq
...
for i = 1:MAXIT
    ...
    if t>= t_plot[pl_idx] #|| i==Nsteps
        # @show t
        if save_data
            filename = string("malpasset_h",string(N),"_",string(K1D),"_",pl_idx,".csv");
            writedlm( filename,  h, ',');
